...
class Route():
    def __init__(self):
        ...
        self.pub_go_to = rospy.Publisher("/go_to",String,queue_size=10)
    ...
    def go_to(self,x1,y1):
        ...
        while(target_time>rospy.get_time()):
            ...
            self.pub_cmd.publish(self.twist)
            if(self.x >x1-const.POSITION_ERROR and self.x<x1+const.POSITION_ERROR):
                if(self.y>y1-const.POSITION_ERROR and self.y<y1+const.POSITION_ERROR):
                    break

        self.pub_go_to.publish("Arrived")
        self.arrived=True
        self.twist.linear.x=0
        self.twist.angular.z=0
        self.pub_cmd.publish(self.twist)
